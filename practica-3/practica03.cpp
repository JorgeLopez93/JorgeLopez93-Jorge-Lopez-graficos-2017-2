...
bool flag_ini = 0, SphereFlag = true, normFlag = 0, textureFlag = 0;
int fogSelector = 0;
...
void init(){
  ...
  glGenBuffers(1, &floor_buffer);
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(floor_points)*3 + sizeof(floor_colors) + sizeof(floor_texCoord),
		NULL, GL_STATIC_DRAW);
	...
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(floor_points)*3, sizeof(floor_colors), floor_colors);
  ...
  glGenBuffers(1, &x_buffer);
	...
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(x_points)*3, sizeof(x_colors), x_colors);
  ...
  glGenBuffers(1, &y_buffer);
	...
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(y_points)*3, sizeof(y_colors), y_colors);
  ...
  programIni = InitShader("vshader1.glsl", "fshader1.glsl");
...
void drawObj(GLuint buffer, int num_vertices, bool flag, bool texFlag)
{
    glBindBuffer(GL_ARRAY_BUFFER, buffer);
    GLuint vNormal, vPosition, vColor, vTexCoord;
    ...
    if (flag) {
      ...
      if (texFlag) {
        ...
        glVertexAttribPointer( vTexCoord, 2, GL_FLOAT, GL_FALSE, 0,
          BUFFER_OFFSET((sizeof(point3)*num_vertices*3) + (sizeof(color4)*num_vertices)));
      }
      ...
      BUFFER_OFFSET(sizeof(point3) * num_vertices*3) );
      if (texFlag) {
        vTexCoord = glGetAttribLocation( programIni, "vTexCoord" );
        glEnableVertexAttribArray( vTexCoord );
        glVertexAttribPointer( vTexCoord, 2, GL_FLOAT, GL_FALSE, 0,
          BUFFER_OFFSET((sizeof(point3)*num_vertices*3) + (sizeof(color4)*num_vertices)));
      }
...
void display( void ){
  ...
  if (ilum) {
    ...
    glUniform1i(glGetUniformLocation(program, "fogSelector"), fogSelector);
    ...
    glUniform1i(glGetUniformLocation(programIni, "fogSelector"), fogSelector);
    glUniform1i( glGetUniformLocation(programIni, "texture_2D"), 0 );
  }
  ...
  if(ilum){
    ...
    glUniform4f(glGetUniformLocation(program, "fogColor"), 0.7, 0.7,0.7, 0.5);
    glUniform1f(glGetUniformLocation(program, "fogDensity"), 0.09f);

  }else{
    glUniform1i(glGetUniformLocation(programIni, "Texture_app_flag"), 0);
    glUniform4f(glGetUniformLocation(programIni, "fogColor"), 0.7, 0.7,0.7, 0.5);
    glUniform1f(glGetUniformLocation(programIni, "fogDensity"), 0.09f);
  }
  ...
  drawObj(sphere_buffer, numNodes, ilum, 0);

  m = Translate(0.0, 0.05, 0.0);
  im = mat4WithUpperLeftMat3(inverse(upperLeftMat3(m)));
  mat4 mv = LookAt(eye, VPN, VUP) * Translate(0.0, 0.02, 0.0);
  ...
  if(ilum){
	...
	drawObj(x_buffer, 3, ilum, 0);
	drawObj(y_buffer, 3, ilum, 0);
	drawObj(z_buffer, 3, ilum, 0);
  ...
  if (textureFlag) {
    if (ilum) {
      ...
      glUniform1i(glGetUniformLocation(programIni, "Texture_app_flag"), 1);
    }
  }
  drawObj(floor_buffer, 6, ilum, 1) ;
